...
## Dual Runner (Same grammY Handlers)

The same bot handlers can run in two modes:

- **Prod webhook mode** (`/api/bot`): grammY `webhookCallback` in Vercel serverless.
- **Local polling mode** (no ngrok): grammY `bot.start()` with getUpdates.
...
Set env vars (`BOT_TOKEN`, `TELEGRAM_WEBHOOK_URL`, optional `TELEGRAM_WEBHOOK_SECRET`) and run:
...
Do not run polling locally while webhook is active in prod with the same bot token.
